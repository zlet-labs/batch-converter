...
По текущей справке Google Gemini Notebook поддерживает DOCX, PPTX, TXT, Markdown, PDF, CSV, EPUB и ряд форматов изображений. XLS/XLSX и TSV как прямые upload-форматы в этом списке не указаны, поэтому TSV в Zlet Converter позиционируется как универсальный табличный экспорт, а не как обещание совместимости Gemini Notebook.
