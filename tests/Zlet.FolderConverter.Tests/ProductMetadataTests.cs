...
public sealed class ProductMetadataTests
{
    ...
    public void App_assembly_uses_public_product_identity_and_version()
    {
        ...
        Assert.Equal(new Version(0, 0, 0, 0), AppAssembly.GetName().Version);
    ...
    public void App_assembly_exposes_display_and_portable_package_names()
    {
        ...
        Assert.Equal("v0.0.0", metadata["DisplayVersion"]);
        Assert.Equal("ZletBatchConverter-v0.0.0-win-x64", metadata["PortablePackageName"]);
    }
    ...
    public void Public_identity_drives_ui_and_result_zip_names()
    {
        ...
        Assert.Equal("0.0.0", global::Zlet.FolderConverter.App.ProductIdentity.Version);
        Assert.Equal(
            "ZletBatchConverter-v0.0.0-results.zip",
    ...
    public void Repository_documents_use_new_product_but_keep_current_repository_url()
    {
        ...
        Assert.Contains("v0.0.0", readme);
        Assert.Contains("https://github.com/zlet-labs/folder-converter", readme);
        Assert.Contains("ZletBatchConverter-v0.0.0-win-x64.zip", portableReadme);
